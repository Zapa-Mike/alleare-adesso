...
@Component({
  ...
  template: `
    ...
    <div class="grid-containerAntworten">
      ...
      <button id={{antwort3anzeige}} class="antwort shadow" (click)="push($event)">{{antwort3anzeige}}</button>
      <button id={{antwort4anzeige}} class="antwort shadow" (click)="push($event)">{{antwort4anzeige}}</button>
    </div>
    <div class="col rowVZ">
